...
class PharmaDataProcessor:
    ...
    def _prepare_data(self):
        try:
                ...
                for pattern in patterns:
                    for col in self.df.columns:
                        if pattern in str(col):
                            column_mapping[col] = pattern.replace('\n', ' ').strip()
            ...
            metric_types = ['USD MNF', 'Standard Units', 'Units', 'SU Avg Price USD MNF', 'Unit Avg Price USD MNF']
            ...
            for year in year_metrics:
                for metric in metric_types:
                    ...
                    if col_name in self.df.columns:
                        ...
                        # Handle different decimal separators and formatting
            ...
            print(f"Numeric columns processed: {numeric_columns}")
            ...
            text_columns = [
                ...
                'International Prescription', 'Prescription Status'
            ]
            ...
            for col in text_columns:
                if col in self.df.columns:
                    ...
                    self.df[col] = self.df[col].astype(str).fillna('Bilinmiyor').str.strip()
            
            print(f"Text columns processed: {available_text_cols}")
            
            # Create year columns for easier access
        ...
        try:
            for year, suffix in [(2022, 'MAT Q3 2022'), (2023, 'MAT Q3 2023'), (2024, 'MAT Q3 2024')]:
                ...
                if usd_col in self.df.columns:
                    ...
                    alt_usd_col = f"{suffix} USD MNF"
                    if alt_usd_col in self.df.columns:
                        self.df[f'USD_{year}'] = self.df[alt_usd_col]
    ...
    def _standardize_country_names(self):
        ...
        if 'Country' in self.df.columns:
            self.df['Country'] = self.df['Country'].str.upper().replace(country_mapping)
        elif 'Source.Name' in self.df.columns:
            # Try to extract country from Source.Name
            self.df['Country'] = self.df['Source.Name'].apply(
                lambda x: country_mapping.get(str(x).split('.')[0].upper(), str(x).split('.')[0])
                if isinstance(x, str) and '.' in x else 'Bilinmiyor'
            )
    ...
    def _categorize_specialty(self):
        if 'Specialty Product' in self.df.columns:
            self.df['Specialty_Type'] = self.df['Specialty Product'].apply(
                lambda x: 'Specialty' if str(x).upper() in ['YES', 'EVET', 'TRUE', '1', 'SPECIALTY', 'SPECIAL'] else 'Non-Specialty'
            )
        else:
            self.df['Specialty_Type'] = 'Non-Specialty'
    ...
    def _calculate_metrics(self):
        try:
            ...
            print(f"Error calculating metrics: {traceback.format_exc()}")
    ...
    def _calculate_growth_metrics(self):
        ...
        if 'USD_2022' in self.df.columns and 'USD_2024' in self.df.columns:
            mask = self.df['USD_2022'] > 0
            self.df.loc[mask, 'CAGR'] = (
                (self.df.loc[mask, 'USD_2024'] / self.df.loc[mask, 'USD_2022']) ** (1/2) - 1
            ) * 100
    ...
    def _calculate_price_volume_decomposition(self):
        for year in [2023, 2024]:
            ...
            if all(col in self.df.columns for col in [usd_current, usd_previous, units_current, units_previous, price_current, price_previous]):
                mask = (self.df[price_previous] > 0) & (self.df[units_current].notna())
                self.df.loc[mask, f'Volume_Effect_{year}'] = (
                    (self.df.loc[mask, units_current] - self.df.loc[mask, units_previous]) * self.df.loc[mask, price_previous]
                )
                self.df.loc[mask, f'Price_Effect_{year}'] = (
                    (self.df.loc[mask, price_current] - self.df.loc[mask, price_previous]) * self.df.loc[mask, units_current]
                )
                self.df.loc[mask, f'Mix_Effect_{year}'] = (
                    self.df.loc[mask, usd_current] - self.df.loc[mask, usd_previous] - 
                    self.df.loc[mask, f'Volume_Effect_{year}'] - self.df.loc[mask, f'Price_Effect_{year}']
                )
    ...
    def _display_sales_trend_chart(self):
        try:
            ...
            if len(growth_rates) > 0:
                fig.add_trace(
                    go.Scatter(
                        x=[f"{years[i]}-{years[i+1]}" for i in range(len(growth_rates))],
                        y=growth_rates,
                        name="Yıllık Büyüme (%)",
                        mode='lines+markers',
    ...
    def _generate_insights(self) -> str:
        try:
            ...
            if 'Molecule' in self.df.columns and 'USD_2024' in self.df.columns:
                ...
                if not molecule_sales.empty:
                    total_sales = self.df['USD_2024'].sum()
                    if total_sales > 0:
                        top_molecules = ", ".join([f"{mol} (${sales:,.0f}M)" for mol, sales in molecule_sales.items()])
                        insights.append(f"💊 **Ürün Portföyü:** En büyük 3 molekül: {top_molecules}. Toplam pazarın %{(molecule_sales.sum() / total_sales * 100):.1f}'ini oluşturuyor.")
            ...
            return "<br><br>".join(insights) if insights else "Yeterli veri bulunamadı."
...
class CountryRegionAnalysisTab:
    ...
    def _render_filters(self):
        ...
        with col1:
            available_countries = sorted([c for c in self.df['Country'].unique() if c != 'Bilinmiyor']) if 'Country' in self.df.columns else []
            selected_countries = st.multiselect(
                "Ülkeler",
                options=available_countries,
                default=available_countries[:min(5, len(available_countries))],
                key="country_analysis_countries"
            )
            
        with col2:
            if 'Region' in self.df.columns:
                available_regions = sorted([r for r in self.df['Region'].unique() if r != 'Bilinmiyor'])
                selected_regions = st.multiselect(
                    "Bölgeler",
                    options=available_regions,
                    default=available_regions,
                    key="country_analysis_regions"
                )
            
        with col3:
            if 'Molecule' in self.df.columns:
                available_molecules = sorted([m for m in self.df['Molecule'].unique() if m != 'Bilinmiyor'])
                selected_molecules = st.multiselect(
                    "Moleküller",
                    options=available_molecules,
                    default=available_molecules[:min(3, len(available_molecules))],
                    key="country_analysis_molecules"
                )
    ...
    def _check_data_availability(self) -> bool:
        required_columns = ['Country', 'USD_2024']
            ...
            if 'Molecule' in self.filtered_df.columns and 'Country' in self.filtered_df.columns and 'AvgPrice_Unit_2024' in self.filtered_df.columns:
                ...
                if len(molecule_country_prices['Molecule'].unique()) > 0:
                    selected_molecule = st.selectbox(
                        ...
                        options=sorted([m for m in molecule_country_prices['Molecule'].unique() if m != 'Bilinmiyor']),
                        key="country_molecule_price"
                    )
...
class MoleculeProductAnalysisTab:
        ...
        with col1:
            if 'Molecule' in self.df.columns:
                ...
                selected_molecules = st.multiselect(
                    "Moleküller",
                    options=available_molecules,
                    default=available_molecules[:min(10, len(available_molecules))],
                    key="molecule_analysis_molecules"
                )
            
            if 'Country' in self.df.columns:
                available_countries = sorted([c for c in self.df['Country'].unique() if c != 'Bilinmiyor'])
                selected_countries = st.multiselect(
                    "Ülkeler",
                    options=available_countries,
                    default=available_countries[:min(5, len(available_countries))],
                    key="molecule_analysis_countries"
                )
        
        with col2:
            if 'Chemical Salt' in self.df.columns:
                available_salts = sorted([s for s in self.df['Chemical Salt'].unique() if s != 'Bilinmiyor'])
                selected_salts = st.multiselect(
                    "Kimyasal Tuzlar",
                    options=available_salts,
                    default=available_salts[:min(5, len(available_salts))],
                    key="molecule_analysis_salts"
                )
            
            if 'International Product' in self.df.columns:
                available_products = sorted([p for p in self.df['International Product'].unique() if p != 'Bilinmiyor'])
                selected_products = st.multiselect(
                    "Uluslararası Ürünler",
                    options=available_products,
                    default=available_products[:min(5, len(available_products))],
                    key="molecule_analysis_products"
                )
    ...
    def _display_molecule_growth_trends(self):
        try:
            ...
            molecule_growth = molecule_growth[molecule_growth['count'] >= 3]
            ...
            if all(f'USD_{year}' in self.filtered_df.columns for year in years):
                ...
                with col1:
                    if not emerging_molecules.empty:
                        ...
                        for idx, (molecule, data) in enumerate(emerging_molecules.head(5).iterrows()):
                            st.metric(
                                label=molecule,
                ...
                with col2:
                    if not declining_molecules.empty:
                        ...
                        for idx, (molecule, data) in enumerate(declining_molecules.head(5).iterrows()):
                            st.metric(
                                label=molecule,
                                value=f"${data[f'USD_{years[2]}']:,.1f}M",
                                delta=f"%{data['Growth_Rate']:.1f}",
    ...
    def _render_filters(self):
        ...
        with col1:
            if 'Corporation' in self.df.columns:
                available_corps = sorted([c for c in self.df['Corporation'].unique() if c != 'Bilinmiyor'])
                selected_corps = st.multiselect(
                    "Corporation'lar",
                    options=available_corps,
                    default=available_corps[:min(10, len(available_corps))],
                    key="corporation_analysis_corps"
                )
        ...
        with col2:
            if 'Country' in self.df.columns:
                ...
                selected_countries = st.multiselect(
                    ...
                    options=available_countries,
                    default=available_countries[:min(5, len(available_countries))],
                    key="corporation_analysis_countries"
                )
        ...
        with col3:
            if 'Molecule' in self.df.columns:
                ...
                selected_molecules = st.multiselect(
                    "Moleküller",
                    options=available_molecules,
                    default=available_molecules[:min(5, len(available_molecules))],
                    key="corporation_analysis_molecules"
                )
        ...
        try:
            ...
            if len(molecule_corp_prices['Molecule'].unique()) > 0:
                selected_molecule = st.selectbox(
                    ...
                    options=sorted([m for m in molecule_corp_prices['Molecule'].unique() if m != 'Bilinmiyor']),
                    key="corporation_price_competition_molecule"
                )
    ...
    def _display_price_difference_metrics(self):
        try:
            if 'AvgPrice_Unit_2024' in self.filtered_df.columns:
                ...
                if not np.isnan(specialty_avg_price) and not np.isnan(non_specialty_avg_price) and non_specialty_avg_price > 0:
    ...
    def _display_growth_comparison(self):
        try:
            if 'YoY_Growth_2024' in self.filtered_df.columns:
                ...
                if not np.isnan(specialty_growth) and not np.isnan(non_specialty_growth):
    ...
    def _display_premiumization_metrics(self):
        try:
            if 'AvgPrice_Unit_2024' in self.filtered_df.columns and 'AvgPrice_Unit_2023' in self.filtered_df.columns:
                ...
                if not np.isnan(specialty_price_growth) and not np.isnan(non_specialty_price_growth):
                    st.metric(
            ...
            if 'Units_2024' in self.filtered_df.columns and 'AvgPrice_Unit_2024' in self.filtered_df.columns:
                ...
                fig.add_trace(
                    go.Bar(
                        ...
                        text=[f"${x:.2f}" if not np.isnan(x) else 'N/A' for x in [specialty_avg_price, non_specialty_avg_price]],
                        textposition='auto'
                    ),
                    row=1, col=2
                )
        ...
        with col2:
            if 'Corporation' in self.df.columns:
                ...
                selected_corps = st.multiselect(
                    "Corporation'lar:",
                    options=available_corps,
                    default=available_corps[:min(5, len(available_corps))],
...
class PackStrengthAnalysisTab:
    ...
    def _render_filters(self):
        ...
        with col1:
            if 'Pack' in self.df.columns:
                available_packs = sorted([p for p in self.df['Pack'].unique() if p != 'Bilinmiyor'])
                selected_packs = st.multiselect(
                    "Pack Seçenekleri:",
                    options=available_packs,
                    default=available_packs[:min(5, len(available_packs))],
                    key="pack_analysis"
                )
        ...
        with col2:
            if 'Strength' in self.df.columns:
                available_strengths = sorted([s for s in self.df['Strength'].unique() if s != 'Bilinmiyor'])
                selected_strengths = st.multiselect(
                    "Strength Seçenekleri:",
                    options=available_strengths,
                    default=available_strengths[:min(5, len(available_strengths))],
    ...
    def _display_pack_size_optimization(self):
        try:
            if 'Pack' in self.filtered_df.columns and 'AvgPrice_Unit_2024' in self.filtered_df.columns and 'Units_2024' in self.filtered_df.columns:
                ...
                if not pack_analysis.empty:
                    max_value_idx = pack_analysis['USD_2024'].idxmax()
                    max_volume_idx = pack_analysis['Units_2024'].idxmax()
                    max_price_idx = pack_analysis['AvgPrice_Unit_2024'].idxmax()
                    
                    max_value_pack = pack_analysis.loc[max_value_idx]
                    max_volume_pack = pack_analysis.loc[max_volume_idx]
                    max_price_pack = pack_analysis.loc[max_price_idx]
    ...
    def _display_strength_based_analysis(self):
        try:
            if 'Strength' in self.filtered_df.columns:
                ...
                if not strength_analysis.empty:
                    fastest_growing_idx = strength_analysis['YoY_Growth_2024'].idxmax()
                    highest_value_idx = strength_analysis['USD_2024'].idxmax()
                    
                    fastest_growing = strength_analysis.loc[fastest_growing_idx]
                    highest_value = strength_analysis.loc[highest_value_idx]
    ...
    def _display_form_factor_analysis(self):
        try:
            if 'Pack' in self.filtered_df.columns:
                ...
                if pack_categories:
                    self.filtered_df['Pack_Category'] = self.filtered_df['Pack'].apply(
                    ...
                    if not category_analysis.empty:
                        top_growth_idx = category_analysis['YoY_Growth_2024'].idxmax()
                        top_value_idx = category_analysis['USD_2024'].idxmax()
                        top_volume_idx = category_analysis['Units_2024'].idxmax()
                        
                        top_growth_category = category_analysis.loc[top_growth_idx]
                        top_value_category = category_analysis.loc[top_value_idx]
                        top_volume_category = category_analysis.loc[top_volume_idx]
    ...
    def _categorize_pack_types(self, packs):
        categories = {
            'Tablet': ['tablet', 'tab', 'tbl', 'compressed', 'film coated'],
            'Kapsül': ['capsule', 'cap', 'kapsul', 'caps'],
            'Sıvı': ['liquid', 'syrup', 'solution', 'oral solution', 'suspension', 'drops'],
            'Enjeksiyon': ['injection', 'inj', 'vial', 'ampoule', 'syringe', 'ampul'],
            'Krem': ['cream', 'ointment', 'gel', 'lotion', 'pomad'],
            'Aerosol': ['aerosol', 'inhaler', 'spray', 'puffer', 'nebulizer'],
            'Patch': ['patch', 'transdermal', 'band'],
            'Powder': ['powder', 'granule', 'sachet', 'granulat']
        }
        return categories
...
class InsightEngineTab:
    ...
    def render(self):
        ...
        with col2:
            if st.button("🔍 İçgörüleri Yeniden Hesapla", type="primary", key="recalculate_insights"):
                if 'insights_generated' in st.session_state:
    ...
    def _display_insights(self):
        ...
        with col1:
            ...
            if risk_count > 0:
                st.metric("Risk İçgörüleri", value=risk_count, key="risk_metric")
        
        with col2:
            opportunity_count = sum(1 for insight in self.insights if "FIRSAT:" in insight or "PREMIUM FIRSAT:" in insight)
            if opportunity_count > 0:
                st.metric("Fırsat İçgörüleri", value=opportunity_count, key="opportunity_metric")
        
        with col3:
            growth_count = sum(1 for insight in self.insights if "🚀" in insight or "📈" in insight)
            if growth_count > 0:
                st.metric("Büyüme İçgörüleri", value=growth_count, key="growth_metric")
...
class GlobalFilters:
    ...
    def render_sidebar(self):
        with st.sidebar:
            ...
            if st.button("🔄 Filtreleri Temizle", type="secondary", key="clear_filters"):
                self._clear_filters()
    ...
    def _render_corporation_filter(self):
        if 'Corporation' in self.df.columns:
            available_corporations = sorted([c for c in self.df['Corporation'].unique() if c != 'Bilinmiyor'])
    ...
    def _render_sector_filter(self):
        if 'Sector' in self.df.columns:
            available_sectors = sorted([s for s in self.df['Sector'].unique() if s != 'Bilinmiyor'])
    ...
    def _render_panel_filter(self):
        if 'Panel' in self.df.columns:
            available_panels = sorted([p for p in self.df['Panel'].unique() if p != 'Bilinmiyor'])
    ...
    def _render_specialty_filter(self):
        if 'Specialty_Type' in self.df.columns:
            available_types = sorted([t for t in self.df['Specialty_Type'].unique() if t != 'Bilinmiyor'])
...
class PharmaAnalyticsApp:
    ...
    def _initialize_app(self):
        ...
        uploaded_file = st.sidebar.file_uploader(
            ...
            type=['xlsx', 'xls'],
            help="Sütunlar: Source.Name, Country, Sector, Panel, Region, Sub-Region, Corporation, Manufacturer, Molecule List, Molecule, Chemical Salt, International Product, Specialty Product, NFC123, International Pack, International Strength, International Size, International Volume, International Prescription, MAT Q3 2022 USD MNF, MAT Q3 2022 Standard Units, MAT Q3 2022 Units, MAT Q3 2022 SU Avg Price USD MNF, MAT Q3 2022 Unit Avg Price USD MNF, MAT Q3 2023 USD MNF, MAT Q3 2023 Standard Units, MAT Q3 2023 Units, MAT Q3 2023 SU Avg Price USD MNF, MAT Q3 2023 Unit Avg Price USD MNF, MAT Q3 2024 USD MNF, MAT Q3 2024 Standard Units, MAT Q3 2024 Units, MAT Q3 2024 SU Avg Price USD MNF, MAT Q3 2024 Unit Avg Price USD MNF"
        )
