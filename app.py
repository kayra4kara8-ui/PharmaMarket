Konu: PharmaIntelligence Pro v8.0 - Ürün-Paket (ProdPack) ve Molekül Derinlik Analizi"Sana aşağıda verdiğin app.py dosyasını, ilaç sektörü yöneticilerinin 'Hangi molekülde, hangi spesifik ürün/paket (ProdPack) ne kadar satıyor ve pazar payı nasıl değişiyor?' sorusuna anlık yanıt verebileceği, 4000+ satır kalitesinde bir Enterprise Karar Destek Platformuna dönüştürmeni istiyorum.Lütfen şu kritik teknik ve analitik modülleri ekle:1. "ProdPack" Derinlik Analizi (Yeni Modül):Molekül Drill-Down: Bir molekül seçildiğinde, o molekülün altındaki tüm markaları, üreticileri ve spesifik paketleri (ProdPack) satış hacmi ve büyüme oranlarına göre listeleyen bir hiyerarşi kur.Hiyerarşik Görselleştirme: Molekül $\rightarrow$ Şirket $\rightarrow$ Marka $\rightarrow$ Paket akışını gösteren interaktif bir Sunburst veya Sankey diyagramı ekle.Pazar Kanibalizasyonu: Aynı şirket içindeki farklı paketlerin veya markaların birbirinin payından çalıp çalmadığını (Büyüme/Hacim matrisi ile) analiz et.2. Teknik Hata Giderme & Performans:Regex Yıl Ayıklama: Sütun isimlerindeki yılları re.search(r'20\d{2}') ile bul. 'PENICILLIN...' gibi metin içeren sütunların int() hatası vermesini engelle.Benzersiz İsimlendirme: 'Bölge' (Region) ve 'Alt Bölge' (Sub-Region) gibi sütunlar temizlendiğinde aynı ismi alıyorsa, bunları otomatik olarak Bölge, Bölge_1 şeklinde benzersizleştir.Güvenli Optimizasyon: Downcast işlemlerinde karşılaşılan 'Ambiguous Truth Value' hatasını pd.api.types kullanarak çöz.Hacim Kapasitesi: Veri gezginindeki gösterim limitini 5.000 satıra çıkar ve 1M+ satır için st.cache_data optimizasyonunu en üst seviyeye çek.3. İleri Seviye AI ve Stratejik Öngörü:Tahminleme (2025-2026): statsmodels (Holt-Winters) ile pazarın gelecek 2 yılını tahmin et ve 'Yatırım Tavsiyesi' kutuları ekle.Risk ve Fırsat İzleme: IsolationForest ile pazar normlarından sapan, aşırı büyüyen veya kritik düşüş yaşayan paketleri (ProdPack) anlık tespit et.PCA Tabanlı Segmentasyon: Ürünleri pazar payı, fiyat esnekliği ve büyüme hızı verileriyle PCA ve K-Means kullanarak 'Liderler', 'Potansiyeller' ve 'Riskli Ürünler' olarak grupla.4. Kurumsal UI/UX:Temayı 'Executive Dark Mode' (Lacivert, Gümüş ve Altın tonları) yap.Grafiklerin altına otomatik 'Yönetici Özeti' (Insight Box) ekle. (Örn: 'X molekülünde Y paketi son 12 ayda pazarın %30'unu domine etti, ancak büyüme hızı yavaşlıyor.')Çıktı Gereksinimi:En başta güncel bir requirements.txt içeriği sağla.Ardından, tüm hatalardan arınmış, modüler ve yaklaşık 4000 satır derinliğinde tek parça app.py kodunu ver." ["""
